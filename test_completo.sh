...
echo "1. CRIAR LOJA"
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X POST "$BASE/stores" \
...
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X POST "$BASE/products" \
  -H "Content-Type: application/json" \
...
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X POST "$BASE/inventories" \
  -H "Content-Type: application/json" \
...
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/users?email=cliente@teste.com&page=0&size=1")
BODY=$(cat "$RESP_FILE")
...
if [ -n "$STORE_ID" ] && [ -n "$PROD_1_ID" ] && [ "$STORE_ID" != "null" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X POST "$BASE/orders" \
    -H "Content-Type: application/json" \
...
if [ -n "$ORDER_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X PATCH "$BASE/orders/$ORDER_ID/status" \
    -H "Content-Type: application/json" \
    -d '{"status":"APPROVED"}')
  BODY=$(cat "$RESP_FILE")
...
if [ -n "$ORDER_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X POST "$BASE/pickup-authorizations" \
    -H "Content-Type: application/json" \
...
if [ -n "$PROD_1_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/products/$PROD_1_ID")
...
if [ -n "$STORE_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/stores/$STORE_ID")
...
if [ -n "$ORDER_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/orders/$ORDER_ID")
...
if [ -n "$STORE_ID" ] && [ -n "$PROD_1_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/inventories/store/$STORE_ID/product/$PROD_1_ID")
...
if [ -n "$AUTH_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/pickup-authorizations/$AUTH_ID")
...
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/products?page=0&size=10")
check "GET /products (paginado)" 200 "$HTTP"

HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/stores?page=0&size=10")
check "GET /stores (paginado)" 200 "$HTTP"

HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/orders?page=0&size=10")
check "GET /orders (paginado)" 200 "$HTTP"

HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/inventories?page=0&size=10")
check "GET /inventories (paginado)" 200 "$HTTP"

HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/notifications?page=0&size=10")
check "GET /notifications (paginado)" 200 "$HTTP"

if [ -n "$ORDER_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/pickup-authorizations/order/$ORDER_ID?page=0&size=10")
...
if [ -n "$PROD_1_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X PUT "$BASE/products/$PROD_1_ID" \
    -H "Content-Type: application/json" \
...
if [ -n "$STORE_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X PUT "$BASE/stores/$STORE_ID" \
    -H "Content-Type: application/json" \
...
if [ -n "$STORE_ID" ] && [ -n "$PROD_1_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X PUT "$BASE/inventories/store/$STORE_ID/product/$PROD_1_ID" \
    -H "Content-Type: application/json" \
...
if [ -n "$ORDER_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/notifications?orderId=$ORDER_ID")
  BODY=$(cat "$RESP_FILE")
...
if [ -n "$NOTIF_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X PATCH "$BASE/notifications/$NOTIF_ID/read")
...
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" "$BASE/products/abc")
BODY=$(cat "$RESP_FILE")
check "GET /products/abc path inválido" 400 "$HTTP" "$BODY"

echo "25. PUT /inventories/store/x/product/1 — storeId não numérico"
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X PUT "$BASE/inventories/store/x/product/1" \
  -H "Content-Type: application/json" \
  -d '{"quantity":10}')
BODY=$(cat "$RESP_FILE")
...
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X PATCH "$BASE/orders/9999/status" \
  -H "Content-Type: application/json" \
  -d '{"status":"CANCELED"}')
BODY=$(cat "$RESP_FILE")
...
HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/products/9999")
BODY=$(cat "$RESP_FILE")
...
if [ -n "$INV_1_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/inventories/$INV_1_ID")
...
if [ -n "$INV_2_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/inventories/$INV_2_ID")
...
if [ -n "$AUTH_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/pickup-authorizations/$AUTH_ID")
...
if [ -n "$ORDER_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/orders/$ORDER_ID")
...
if [ -n "$PROD_2_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/products/$PROD_2_ID")
...
if [ -n "$PROD_1_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/products/$PROD_1_ID")
...
if [ -n "$STORE_ID" ]; then
  HTTP=$(curl -s -o "$RESP_FILE" -w "%{http_code}" -X DELETE "$BASE/stores/$STORE_ID")
